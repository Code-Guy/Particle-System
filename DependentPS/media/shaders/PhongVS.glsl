...
out vec3 T_cameraspace;
out vec3 B_cameraspace;
...
void main()
{
	...
	Normal_cameraspace = mat3(N) * vertexNormal_modelspace; // Only correct if ModelMatrix does not scale the model ! Use its inverse transpose if not.
	T_cameraspace = mat3(N) * T;
	B_cameraspace = mat3(N) * B;
