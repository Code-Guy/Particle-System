...
template <typename type> struct box3
{
	...
	bool contains(const box3<type> &p) const
	{
		return p.xmin >= xmin && p.xmax <= xmax && p.ymin >= ymin && p.ymax <= ymax && p.zmin >=zmin && p.zmax <= zmax;
	}

	bool overlap(const box3<type> &p) const
	{
		if (abs(xmin - p.xmin) > (xmax - xmin + p.xmax - p.xmin) * 0.5)
		{
			return false;
		}
		if (abs(ymin - p.ymin) > (ymax - ymin + p.ymax - p.ymin) * 0.5)
		{
			return false;
		}
		if (abs(zmin - p.zmin) > (zmax - zmin + p.zmax - p.zmin) * 0.5)
		{
			return false;
		}
		return true;
