...
public:
	...
	/************************************************************************/
	/* xuyibo rotate the vector                                             */
	/************************************************************************/
	vec3 rotateVec(float angle, float xp, float yp, float zp) const
	{
		float new_x;
		float new_y;
		float new_z;
		...
		new_x  = (xp*xp*(1-c) + c)	* x;
		new_x += (xp*yp*(1-c) - zp*s)	* y;
		new_x += (xp*zp*(1-c) + yp*s)	* z;

		new_y  = (yp*xp*(1-c) + zp*s)	* x;
		new_y += (yp*yp*(1-c) + c)	* y;
		new_y += (yp*zp*(1-c) - xp*s)	* z;

		new_z  = (xp*zp*(1-c) - yp*s)	* x;
		new_z += (yp*zp*(1-c) + xp*s)	* y;
		new_z += (zp*zp*(1-c) + c)	* z;
