...
void FrameBuffer::Parameters::set(const Parameters &p)
{
    ...
    if (pointId != p.pointId)
    {
		if (p.pointSize <= 0.0)
		{
			glEnable(GL_PROGRAM_POINT_SIZE, true);
		}else{
...
void FrameBuffer::drawFeedback(ptr<Program> p, MeshMode m, const TransformFeedback &tfb, const MeshBuffers &mesh, int stream)
{
    assert(TransformFeedback::TRANSFORM == NULL);
    set();
    p->set();
